...
/**
 * 1.CAS:旧的值A,预期值B,修改值C,只有当A与B相同时才修改为C,否则不做任何修改；多线程同时操作时，只有一个线程能修改成功，是乐观锁、自旋锁
 * 2.CAS是原子操作，是CPU的一个指令，自旋时间过长，消耗CPU资源，如果资源竞争激烈，多线程自旋长时间消耗资源
 * 3.JDK1.7时采用segment分段加锁的方式，对部分加锁；1.8之后采用sychronized+CAS的加锁方式
 * 4.get操作可以无锁是由于Node的元素val和指针next是用volatile修饰的，在多线程环境下线程A修改结点的val或者新增节点的时候是对线程B可见的
