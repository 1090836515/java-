 ...
 *      重写了equals(重写前效果等同于==，重写后比较两个对象的值，==在基本数据类型时比较的是值，在引用类型时比较的是地址)，则必须
 ...
 * 9.TreeMap的底层数据结构是红黑树，TreeMap的key不能为null（如果为null，那还怎么排序呢），TreeMap有序是通过Comparator来进行
 *      比较的，如果comparator为null，那么就使用自然顺序，是一种平衡二叉查找树
 */
