 ...
 * 4.throw new xxxException：抛出指定的异常.必须定义在方法内部；对象必须是Exception或其子类；必须处理这个对象（RuntimeException或其子类除外,
 *      不用处理，会默认交给JVM处理）；
 ...
 *      用于程序员自行产生并抛出异常
 *      位于方法体内部，可以作为单独语句使用
 *      如果异常对象是非 RuntimeException 则需要在方法申明时加上该异常的抛出，即需要加上 throws 语句 或者 在方法体内 try catch 处理该异常，否
 *          则编译报错。
